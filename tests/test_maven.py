...
class MavenTest(TestCase):
    def test_interpolate_syscall(self):
        ...
        #pom = env.project("org.scijava", "pom-scijava").at_version("35.1.1").pom(interpolated=True)
        #self.assert_pom(pom)

        # Interpolate with SysCallResolver.
        env.resolver = maven.SysCallResolver("mvn")
        env.resolver.mvn_flags = ["-o"] + env.resolver.mvn_flags
        pom_syscall = env.project("org.scijava", "pom-scijava").at_version("35.1.1").pom(interpolated=True)
